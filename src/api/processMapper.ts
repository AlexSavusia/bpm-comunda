...
// По твоим данным: template_noop подходит всем ("nodeKey": ".*")
...
export function apiDefinitionToDiagramSchema(def: ApiProcessDefinition): DiagramSchema {
    const nodes: DiagramNode[] = def.nodes.map((n: ApiProcessNode) => ({
        id: n.id,
        type: backendKeyToNodeType(n.key), // ✅ key определяет тип узла
        name: n.name ?? "",
...
export function diagramSchemaToApiDefinition(schema: DiagramSchema, rootNodeId?: string): ApiProcessDefinition {
    const nodes = schema.nodes.map((n) => {
        const backendKey = n.nodeKey; // из metadata
        if (!backendKey) throw new Error("nodeKey missing");


        return {
            key: backendKey, // ✅ ВАЖНО: именно backend key, иначе 400
            id: n.id,
            name: n.name,
            description: "",
            ...
            // ✅ template: пока ставим noop всем (подходит любому узлу)
    ...
    const flows = schema.edges.map((e) => ({
        ...
        fromNodeId: e.from,
        toNodeId: e.to,
        mainFlow: e.mainFlow !== false, // default true
